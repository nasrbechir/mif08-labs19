...
TEST_DIR = HERE
IMPLEM_DIR = HERE
MINIC_COMPILE = os.path.join(IMPLEM_DIR, 'Main.py')

ALL_FILES = glob.glob(TEST_DIR + '/tests*/**/[a-zA-Z]*.c', recursive=True)
if 'TEST_FILES' in os.environ:
    ALL_FILES = glob.glob(os.environ['TEST_FILES'], recursive=True)

ALLOC_FILES = glob.glob(HERE + '/testsalloc/*.c')
...
class TestCodeGen(TestExpectPragmas):
    ...
    def link_and_run(self, output_name, exec_name, info):
        ...
        try:
            ...
            self.fail("Timeout executing program. Infinite loop in generated code?")
