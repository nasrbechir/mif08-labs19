from Operands import Temporary, S, Register, GP_REGS, Offset, FP
from Instruction3A import Instru3A, regset_to_string
...
class SmartAllocator(Allocator):
    ...
    def set_gen_kill(self):
        ...
        for ins in self._f.get_instructions():
            ...
            for arg in ins.args:
                ...
                # _kill or _gen (use ins._is_read_only() to check
                # whether the first operand is read or written to).
                raise NotImplementedError("Set _gen and _kill for instruction")
    ...
    def run_dataflow_analysis(self):
        """Run the dataflow liveness analyis, i.e. compute self._mapin and
    ...
    def interfere(self, t1, t2):
        ...
        the function."""
        for key in self._mapout:
            if ((t1 in self._mapout[key] and t2 in self._mapout[key]) or
                    (t1 in self._mapout[key] and t2 in self._mapdef[key]) or
                    (t2 in self._mapout[key] and t1 in self._mapdef[key])):
                return True
        return False
