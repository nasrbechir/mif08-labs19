...
from SujetBLexer import SujetBLexer
from SujetBParser import SujetBParser
...
usage : python3 sujetB.py <filetotest>
...
def main(inputname, verbose):
    if inputname:
        lexer = SujetBLexer(FileStream(inputname))
    else:
        lexer = SujetBLexer(StdinStream())
    if not verbose:
